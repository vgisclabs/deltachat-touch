...
Page {
    ...
    function encryptDatabaseStage2() {
        ...
        if (numberUnencryptedAccounts > 0) {
            let popup6 = PopupUtils.open(Qt.resolvedUrl("ConfirmDatabaseConversionToEncrypted.qml"), settingsPage)
    ...
    function decryptDatabaseStage1() {
        ...
        if (DeltaHandler.hasEncryptedAccounts()) {
            let popup9 = PopupUtils.open(Qt.resolvedUrl("ConfirmDatabaseConversionToUnencrypted.qml"), settingsPage)
                            ...
                            if (checked != DeltaHandler.databaseIsEncryptedSetting()) {
                                ...
                                if (numberUnconfiguredAccounts === 1) {
                                    ...
                                    if (checked) {
                                        ...
                                        let popup4 = PopupUtils.open(Qt.resolvedUrl("ConfirmEncryptDatabase.qml"), settingsPage)
                                        popup4.confirmed.connect(encryptDatabaseStage1)
                                        popup4.cancelled.connect(uncheckEncryptDatabaseSwitch)
                                    } else {
                                        let popup5 = PopupUtils.open(Qt.resolvedUrl("ConfirmStopEncryptingDatabase.qml"), settingsPage)
